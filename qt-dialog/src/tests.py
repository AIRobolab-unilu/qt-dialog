...
class TestDialog():
                ...
                for result, expected in getattr(self, name)():
                    ...
                    if hasattr(expected, '__iter__'):
                        if result not in expected:
                            rospy.logerr('assert failed {} not in {}'.format(result, expected))
                            good = False
                            break
    ...
    def test_qt(self):
        possibilities = ['cutie what is your name', "what's your name kitty"]
    ...
    def test_create_conv(self):
        ...
        questions = ["let's begin this is conversation", 'begin the discussion']
        for q in questions:

            self.send(q)
            self.wait_until_timeout()

            yield self.result, cst.START
            self.reset_pub.publish(True)

    def test_feeling_good(self):
       
        questions = ["i feel good", "i'm happy"]
        for q in questions:

            self.send(q)
            self.wait_until_timeout()

            yield self.result, cst.FEELING_GOOD
            self.reset_pub.publish(True)

    def test_feeling_bad(self):
       
        questions = ["i am very sad", "i'm not happy"]
        for q in questions:

            self.send(q)
            self.wait_until_timeout()

            yield self.result, cst.FEELING_BAD
            self.reset_pub.publish(True)

    def test_feeling_angry(self):
       
        questions = ["i am angry"]
        for q in questions:

            self.send(q)
            self.wait_until_timeout()

            yield self.result, cst.FEELING_ANGRY
            self.reset_pub.publish(True)

    def test_robot_feeling(self):
        ...
        for q in questions:
            ...
            yield self.result, cst.ROBOT_FEELINGS
            self.reset_pub.publish(True)

    """def test_end_conv(self):
       
        questions = ["end this is conversation", 'stop the discussion']
        for q in questions:

            self.send('begin the discussion')
            self.wait_until_timeout()
